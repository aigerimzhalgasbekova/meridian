...
-- re-running is a no-op. The DELETE drops any 'undo_email' row left by the
-- version that briefly minted them: nothing reads that purpose any more, and
-- the constraint below would not validate with one present.
DELETE FROM one_time_tokens WHERE purpose = 'undo_email';
