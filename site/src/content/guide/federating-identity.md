...
The authorization-code flow's anti-forgery values are commonly confused with each other; [ADR 0003](https://github.com/aigerimzhalgasbekova/meridian/blob/main/bridge/docs/adr/0003-state-nonce-pkce.md) keeps their jobs distinct:
...
Upstream IdPs are dependencies bridge doesn't operate, and the failure mode to refuse is the slow one: every `/login/{provider}` holding a connection for a 10-second timeout against a dead endpoint, users staring at spinners, sockets piling up. [ADR 0002](https://github.com/aigerimzhalgasbekova/meridian/blob/main/bridge/docs/adr/0002-breaker-and-stale-jwks.md):
